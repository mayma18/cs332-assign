...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
      ...
      yield (i._1, i._2 :: history)).to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty)  LazyList.empty
      ...
      val temp = 
        (for{
          i <- initial
          j <- newNeighborsOnly(neighborsWithHistory(i._1, i._2),explored) 
    } yield j).to(LazyList)
      LazyList.concat(initial,from(temp, explored ++ temp.map((x)=> x._1)))
    }
   
   
   
   
   /*if (initial.length == 0) LazyList.empty
    else{
      val temp = 
        (for{
          i<-initial.indices
          if (i >= initial.length -5)
          k<- newNeighborsOnly(neighborsWithHistory(initial(i)._1, initial(i)._2), explored)
        } yield k).to(LazyList)
      LazyList.concat(initial,from(temp,explored ++ temp.map(x=>x._1)))
    }*/
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = 
    from(((startBlock,Nil)::Nil).to(LazyList), Set(startBlock))
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = 
    pathsFromStart.filter(x=>done(x._1)).to(LazyList)
